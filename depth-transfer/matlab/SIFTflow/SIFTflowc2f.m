...
for i=nlevels:-1:1
    ...
    [height,width,nchannels]=size(pyrd(i).im1);
    %[~,~,nchannels]=size(pyrd(i).im2);
    %[~,yy]=meshgrid(1:width,1:height);
...
%         vy=vx;
        ...
        winSizeX=ones(height,width)*topwsize.x;
        winSizeY=ones(height,width)*topwsize.y;
    else
...
%         winSizeY=decideWinSize(vy,wsize);
        ...
        winSizeX=ones(height,width)*(wsize.x+i-1);
        winSizeY=ones(height,width)*(wsize.y+i-1);
    end
    if nchannels<=3
    ...
    if i==nlevels
        if IsSegmentation
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nTopIterations,2,topwsize.x],vx,vy,winSizeX,winSizeY,Im_s,Im_d);
        else
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nTopIterations,2,topwsize.x],vx,vy,winSizeX,winSizeY);
...
%         end
        ...
        if IsSegmentation
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nIterationArray(i),nlevels-i,wsize.x+i-1],vx,vy,winSizeX,winSizeY,Im_s,Im_d);
        else
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nIterationArray(i),nlevels-i,wsize.x+i-1],vx,vy,winSizeX,winSizeY);
